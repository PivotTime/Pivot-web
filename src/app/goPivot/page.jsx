...
export default function GoPivot() {
  ...
  const [photoCardAlert, setPhotoCardAlert] = useState(true);


  function delayNextStep(nextStep, delay, TransitionState) {
    TransitionState(true);
    setTimeout(() => {
      setPivotStep(nextStep);
      if (nextStep === "done") {
  ...
  const handleSaveImage = () => {
    if (canvasContainerRef.current) {
      html2canvas(canvasContainerRef.current, { backgroundColor: '#000000' }).then(canvas => {
  ...
  const handleCustomObjectSave = useCallback(
    async (newCustomObject) => {
      ...
      try {
          ...
          const updatedTrajectories = prevTrajectories.map((traj, i) => {
            if (i === selectedIndex) {
              return {
                ...traj,
                objectType: "custom",
                customObjectId: newCustomObject.id,
                count: 1, // Force count to 1 for custom objects
              };
            }
            return traj;
          });
        ...
        setFetchError(
          `커스텀 오브젝트를 다시 불러오는 데 실패했습니다: ${error.message}`
        );
      } finally {
  ...
  const RESULT_MESSAGE_TABLE = {
    // ① 선(Line)만 사용
    line: {
      low: `당신은 작은 선택에서도 방향을 잃지 않는 사람입니다.
필요한 만큼만 움직여도 흐름을 또렷하게 만들어냅니다.
당신의 직선은 언제나 앞으로 나아가는 의지를 담고 있습니다.`,

      mid: `당신은 여러 가능성을 정리하며 길을 만들어가는 사람입니다.
복잡한 상황에서도 중심을 잡고, 명확한 결정을 이어갑니다.
당신의 추진력은 주변을 안정시키고 흐름을 전환시킵니다.`,

      high: `당신은 거대한 흐름 속에서도 방향을 잃지 않는 사람입니다.
많은 선택을 감당하면서도 전체의 길을 꿰뚫어봅니다.
당신의 전진은 단순한 움직임을 넘어 하나의 궤적을 만듭니다.`,
    },

    // ② 원(Circle)만 사용
    circle: {
      low: `당신은 작은 순간에도 흐름을 읽어내는 사람입니다.
부드러운 관찰력으로 상황을 조용히 안정시킵니다.
당신의 원은 주변을 연결하고 마음을 편안하게 만듭니다.`,

      mid: `당신은 넓은 시야로 관계와 상황을 조율하는 사람입니다.
흐름 속에서 균형을 찾고, 자연스러운 변화를 이끌어냅니다.
당신의 유연함은 많은 이들에게 안정감을 전합니다.`,

      high: `당신은 다양한 감정을 하나의 흐름으로 묶어내는 사람입니다.
수많은 요소 속에서도 조화로운 형태를 만들어냅니다.
당신의 원은 복잡한 세계를 부드러운 흐름으로 바꿉니다.`,
    },

    // ③ 네모(Square)만 사용
    square: {
      low: `당신은 작은 선택에서도 기준을 세우는 사람입니다.
정돈된 시선으로 필요한 요소만 정확히 골라냅니다.
당신의 구조는 어떤 상황에서도 흔들리지 않는 힘이 됩니다.`,

      mid: `당신은 여러 조각을 정교하게 맞추는 사람입니다.
복잡한 과정을 체계로 묶고, 흐트러진 부분을 안정시킵니다.
당신의 질서는 결과물에 신뢰와 완성도를 더합니다.`,

      high: `당신은 방대한 요소를 하나의 틀로 완성하는 사람입니다.
많은 변수 속에서도 균형을 잃지 않고 구조를 유지합니다.
당신의 체계는 큰 흐름을 견고하게 지탱하는 기반이 됩니다.`,
    },

    // ④ 선 + 원
    "line+circle": {
      low: `당신은 작은 선택 속에서도 길과 흐름을 함께 읽는 사람입니다.
앞으로 나아가되 주변의 움직임을 세심하게 받아들입니다.
당신의 균형 감각은 상황을 자연스럽게 전환시키는 힘이 됩니다.`,

      mid: `당신은 방향과 조화를 동시에 고려하며 움직이는 사람입니다.
계획과 감각이 부드럽게 맞물려 안정된 흐름을 만듭니다.
당신의 리듬은 복잡한 과정도 편안하게 이어가게 합니다.`,

      high: `당신은 넓은 시야로 흐름을 설계하며 움직이는 사람입니다.
많은 요소를 조합하면서도 중심의 방향을 잃지 않습니다.
당신의 선택들은 거대한 흐름을 하나의 궤적으로 만들어냅니다.`,
    },

    // ⑤ 선 + 네모
    "line+square": {
      low: `당신은 단단한 기준 위에서 정확하게 결정을 내리는 사람입니다.
필요한 만큼 움직여도 일의 중심이 선명하게 드러납니다.
당신의 추진력은 흔들림 없이 앞으로 나아가게 합니다.`,

      mid: `당신은 구조 속에서 속도와 완성도를 챙기는 사람입니다.
체계적인 사고와 추진력이 균형을 이루며 결과를 만듭니다.
당신의 흐름은 과정 전체에 안정감과 질서를 가져옵니다.`,

      high: `당신은 방대한 흐름 속에서도 방향을 유지하는 사람입니다.
많은 요소들이 모여도 전체를 정리하며 길을 만들어냅니다.
당신의 전진은 거대한 체계를 움직이는 힘이 됩니다.`,
    },

    // ⑥ 원 + 네모
    "circle+square": {
      low: `당신은 부드러움과 단단함을 함께 지닌 사람입니다.
상황을 섬세하게 받아들이면서도 중심의 기준을 지켜냅니다.
당신의 조화는 주변을 편안하게 만들며 안정된 방향을 줍니다.`,

      mid: `당신은 감성과 논리를 사용해 형태를 만들어가는 사람입니다.
흐름을 읽으면서도 구조를 세워 결과의 완성도를 높입니다.
당신의 균형은 복잡한 과정도 자연스럽게 이어지게 합니다.`,

      high: `당신은 많은 요소 속에서도 조화로움을 만드는 사람입니다.
감성적인 흐름 위에 정밀한 틀을 쌓아 전체를 정리합니다.
당신의 선택은 세밀하지만 넓은 흐름을 담아냅니다.`,
    },

    // ⑦ 선 + 원 + 네모
    "line+circle+square": {
      low: `당신은 작은 선택만으로도 세 가지 힘을 발휘하는 사람입니다.
상황을 읽고 기준을 세우며, 필요한 속도로 움직입니다.
당신의 조합은 단단한 중심과 흐름을 동시에 만들어냅니다.`,

      mid: `당신은 자연스럽게 조율하며 성장하는 사람입니다.
방향과 구조, 흐름이 맞물려 과정 전체를 안정시킵니다.
당신의 움직임은 복잡함도 질서 있게 이어가게 합니다.`,

      high: `당신은 모든 요소를 균형 있게 다루는 사람입니다.
많은 선택이 모여도 중심을 놓치지 않고 형태를 만들어냅니다.
당신의 궤적은 넓고 깊으며, 큰 전환을 이끌어냅니다.`,
    },
  };
  ...
  const getPivotMessage = useCallback(() => {
    // 1) 도형별 총 개수 & 전체 개수 계산
    const shapeCounts = { line: 0, circle: 0, square: 0 };
    let totalCount = 0;
    ...
    trajectories.forEach((t) => {
      if (
        ...
        shapeCounts[t.objectType] += t.count;
        totalCount += t.count;
      }
    });
    ...
    // 아무 도형도 없는 경우
    if (totalCount === 0) {
      return "도형을 조금 더 만들어야 결과를 볼 수 있어요.";
    }

    // 2) 어떤 도형이 쓰였는지 -> 조합 key 생성
    const usedShapes = Object.entries(shapeCounts)
      .filter(([_, cnt]) => cnt > 0)
      .map(([shape]) => shape)
      .sort(); // 조합 안정화를 위해 정렬

    const comboKey = usedShapes.join("+"); // ex) "line", "line+circle", "circle+square"

    // 3) 전체 개수로 'low', 'mid', 'high' 구간 나누기
    let bucketKey = null;
    if (totalCount >= 1 && totalCount <= 11) {
      bucketKey = "low";
    } else if (totalCount >= 12 && totalCount <= 49) {
      bucketKey = "mid";
    ...
    // 예외 처리
    if (!RESULT_MESSAGE_TABLE[comboKey]) {
      return "이 조합은 아직 준비되지 않았어요. 조금만 더 실험해볼까요?";
    }
    if (!RESULT_MESSAGE_TABLE[comboKey][bucketKey]) {
      return "해당 구간의 메시지가 아직 정의되지 않았어요.";
    }

    // 4) 결과 메시지 추출
    let message = RESULT_MESSAGE_TABLE[comboKey][bucketKey];

    // makerName 변수를 사용 중이면 매칭하기
    if (makerName) {
  ...
  const getPivotTag = useCallback(() => {
    ...
    const scores = predefinedTags.reduce(
      (acc, tag) => ({ ...acc, [tag]: 0 }),
      {}
    );
    ...
    if (!selectedTag) {
      ...
      const dominantType = activeTypes.sort(
    ...
    return (
      selectedTag ||
      predefinedTags[Math.floor(Math.random() * predefinedTags.length)]
    );
  }, [trajectories]);
  useEffect(() => {
  ...
  }, [submitData, setInteractionState, pivotStep]); // Added pivotStep
    ...
    <WindowIntroWrapper
      ...
      children={
        <>
          ...
          {pivotStep === "intro" && (
            <div className={` NavBox ${introTransition ? `fadeOut` : ``} ${pivotStep==="done" ? `fadeIn` : ``}`}>
          ...
          >
            {pivotStep === "intro" && (
              <div className="goPivotHero">
                <div className={`titleBox ${introTransition ? `slideUp` : ``}`}>
                  <svg
                    viewBox="0 0 592 90"
                    fill="none"
                ...
                <button
                  className={`startBtn ${introTransition ? `fadeOut` : ``}`}
                  onClick={() => {
                    delayNextStep("make", 1500, setIntroTransition);
                ...
                <Link
                  href="/gpArchive"
                  className={`startBtn goArchive ${
                    introTransition ? `fadeOut` : ``
                  }`} // 버튼 스타일을 여기로 옮기기
                >
                  PIVOT ARCHIVE
                </Link>
                <div className="samplePivotPlace">
                  <svg
                    className={`A ${introTransition ? `slideUp` : ``}`}
                    viewBox="0 0 326 334"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                  ...
                  <svg
                    className={`B ${introTransition ? `slideUp` : ``}`}
                    viewBox="0 0 428 433"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                  ...
                  <svg
                    className={`C ${introTransition ? `slideUp` : ``}`}
                    viewBox="0 0 535 451"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                  ...
                  <svg
                    className={`D ${introTransition ? `slideUp` : ``}`}
                    viewBox="0 0 372 372"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                ...
                <div className={`infoFade ${introTransition ? `fadeOut` : ``}`}>
                  <InfoBox />
                </div>
              </div>
            )}
            {pivotStep === "make" && (
              <div
                className={`makePivot ${makeTransition ? "Transition" : ""}`}
              >
                <div className="topBar">
                  <div
                    className="backBtn"
                    onClick={() => setPivotStep("intro")}
                  >
                    <svg
                      style={{
                ...
                {makePivotModal && (
                  <div className="modal-overlay">
                    <div className="bg" onClick={() => setMakePivotModal(false)}>

                    </div>
                    <div className="modal-content">
                      <div className="modal-top-bar">
                        <p className="title">CUSTOM PIVOT</p>
                      <button
                        className="modal-close-btn"
                        onClick={() => setMakePivotModal(false)}
                      >
                        <svg
                          style={{
                            width: "clamp(15px, 1.51vw, 29px)",
                            height: "clamp(15px, 1.51vw, 29px)",
                          }}
                          viewBox="0 0 29 29"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M0.353516 0.353516L28.3535 28.3535"
                            stroke="#E1E1E1"
                          />
                          <path
                            d="M28.3535 0.353516L0.353513 28.3535"
                            stroke="#E1E1E1"
                          />
                        </svg>
                      </button>
                ...
                <div className="PivotInfoPlaceHolder">
                  <div className="leftLine">
                    ...
                    <div className="mouseBox">
                      ...
                      <div className="middleLine"></div>
                      <div className="mouseInteraction space">
                        <p className="spaceBox">SPACE</p> 
                        <p className="text">고정 / 해제</p>
                      </div>
                    </div>
                    <ControlBox
                      name={"SYSTEM"}
                            ...
                            <>
                              ...
                              <p className="Property">
                                {trajectories[selectedIndex].objectType
                                  .charAt(0)
                                  .toUpperCase() +
                                  trajectories[selectedIndex].objectType.slice(
                                    1
                                  )}
                              </p>
                            </>
                    ...
                    <ControlBox
                      ...
                      element={
                        <div className="shapeGrid">
                          ...
                          <div
                            className={shape === "custom" ? "active" : ""}
                            onClick={() => {
                              setMakePivotModal(true);
                              setShape("custom");
                            }}
                          >
                            <svg
...
<path d="M5.42253 0.570654L0.569923 0.570654L0.569922 12.9207L6.74492 12.9207L12.9199 12.9207L12.9199 8.06805" stroke="#9A9A9A" strokeWidth="1.14"/>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setGeneratedTag(tag);
                        if(makerName)
                          {delayNextStep("done", 800, setMakeTransition);}
                        else{
                          alert("이름을 입력 해 주세요!")
                        }
                ...
                <div className="goPivotDone" style={{ color: "white" }}>
                  <div className="ResultContainer">
                    <div className="TitleText">
                      <p className="MakersPivot">
                        {`${makerName}'s`} <br />
                        PIVOT TIME
                      </p>
                      <p className="Result">{pivotMessage}</p>
                      <div className="buttonList">
                        <div className="btn" onClick={handleSaveImage}>
                          저장하기
                        </div>
                        <Link
                          href="/gpArchive"
                        ...
                        <div className={`eventCard ${photoCardAlert ? ``: `off`  }`}>
                          <p>
                            오프라인전시(11.21) 한정 <br></br>Pivot Time
                            포토카드를 받아가세요!
                          </p>
                          <svg
                            onClick={ ()=> setPhotoCardAlert(false)}
                            width="12"
                            height="12"
                            viewBox="0 0 12 12"
                            fill="none"
                          ...
                          >
                            <g opacity="0.5">
                              <path
                                d="M11.3535 0.354492L0.353516 11.3537"
                                stroke="white"
                              />
                              <path
                                d="M11.3535 11.3535L0.354345 0.353515"
                                stroke="white"
                              />
                            </g>
                          </svg>
              ...
              >
                ...
                <defs>
                  ...
                  >
                    ...
                    {/* stopColor / stopOpacity → camelCase */}
