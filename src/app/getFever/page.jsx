...
function buildAnswersByGroup() {
  ...
  archiveResponses.forEach((person) => {
    person.answers.forEach((entry) => {
      ...
      // groupId에 따라 적절한 nickName 필드 선택
...
export default function GetFever() {
  ...
  useEffect(() => {
    ...
    if (!skipHeroIntro) {
      // [수정 없음] 
      // skipIntro가 false일 때 (처음 방문 시)
      // 1. Hero -> Archive 자동 스크롤 실행
      ...
      // [수정됨]
      // skipIntro가 true일 때 (되돌아올 때)
      // 1. CSS 변수 값을 애니메이션 없이 즉시 최종 상태로 설정 (레이아웃 깨짐 방지)
      gsap.set(":root", {
        "--object-translate-x": "0%",
        "--title-block-translate-x": "0%",
        "--title-block-opacity": "1",
    ...
    // [추가] Step 1이 시작하기 전에 "빈 시간"을 둡니다.
    // 이 구간 동안 사용자는 스크롤하지만 아무 애니메이션도 재생되지 않습니다.
    ...
    // --- 4. [수정] Pin & Scrub (Snap 기능 제거) ---
    scrollTriggerRef.current = ScrollTrigger.create({
      trigger: archiveRef.current,
      pin: true,
      scrub: 0.8, // 스크롤에 1:1로 부드럽게 반응
      end: "+=2000", // "빈 시간"을 포함한 전체 스크롤 길이

      // ↓↓↓ "자기 멋대로 움직이는" 원인이었던 snap 기능 제거 ↓↓↓
      // snap: { ... },

      animation: masterTl,

      // [유지] "빈 시간"이 포함된 타임라인에 맞춘 onUpdate 로직
      onUpdate: (self) => {
    ...
    // ScrollTrigger 생성 후 refresh
    ScrollTrigger.refresh();
    
    // skipHeroIntro일 때 스크롤 위치 재설정
  ...
  const handleSemesterClick = (index) => {
    ...
    // [추가] 
    // SCSS와의 충돌을 막기 위해 애니메이션 직전에 transition을 강제 비활성화
    ...
    gsap
      .timeline({
        ...
        onComplete: () => {
          // [추가]
          // 페이지 이동 전에 transition을 원래대로 복구 (필수는 아니지만 권장)
    ...
    <div className="getFever">
      ...
      <section className="archive-nav" ref={archiveRef}>
        ...
        <div className="archive-title" ref={archiveTitleRef}>
          {/*
            [수정된 부분]
            1. (semester, index) => (...) 를 (semester, index) => { ... } 로 변경
            2. 변수 선언을 map 안으로 이동
            3. ref={el => itemsRef.current[index] = el} 추가
            4. style에서 initialStyles 변수 사용
          */}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  "--semester-radius-factor": initialStyles.radiusFactor ?? 1,
                }}
              >
